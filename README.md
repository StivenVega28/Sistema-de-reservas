...
- Usa `.env.example` como plantilla de configuracion.
- No subas `node_modules`; se instala con `npm install`.
- `package-lock.json` si debe subirse para instalaciones reproducibles.

## Personalizacion

La imagen de fondo del login esta en:

```text
public/img/FondoLogin.jpg
```

El estilo se controla desde:
...
Verifica que MySQL este iniciado y que `.env` tenga credenciales correctas.

### El puerto 4000 esta ocupado

Cambia `PORT` en `.env` o detén el proceso que usa ese puerto.
...
### La imagen del login no cambia

Recarga el navegador con cache limpia usando `Ctrl + F5`.
